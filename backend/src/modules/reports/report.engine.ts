import type { AiIntelligenceResponse } from '../ai/ai-intelligence.engine';
...
function dominantAge(ageGroups: Record<string, number> | undefined) {
  ...
  const top = sorted[0];
  if (!top) return '';
  if (['18-25', '26-40'].includes(top[0])) return 'usía produktif';
...
export function buildReportIntelligence(input: {
  ...
  const verifyPayload = `${reportId}|${version}|${total}|${now.toISOString()}`;
  const docHash = createHash('sha256').update(verifyPayload).digest('hex').slice(0, 16);
  ...
  return {
    ...
    recommendations: {
      priorities: (ai.priorities?.slice(0, 5) ?? []).map((p) => ({
        name: p.name,
        score: p.score,
        explain: p.explain,
      })),
      items: (ai.recommendations?.slice(0, 5) ?? []).map((r) => ({
        title: r.title,
        actions: r.actions,
        priority: r.priority,
      })),
      risks: (ai.riskMatrix?.slice(0, 5) ?? []).map((r) => ({
        name: r.name,
        score: r.score,
        level: r.level,
      })),
      actionPlan: (ai.actionPlan?.slice(0, 5) ?? []).map((a) => ({
        step: a.phase,
        owner: a.items?.[0] ?? '-',
        timeline: a.items?.[1] ?? '-',
      })),
    ...
    auditTrail: [
      { action: 'create', user: createdBy ?? 'system', at: now.toISOString() },
    ],
  };
}
